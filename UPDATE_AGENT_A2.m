...
function [agent, history] = UPDATE_AGENT_A2(AGENT, agent, cue, action, history)

%%% Alternative agent 2:
%   After interval offset, the agent's internal state does not update
...
if cue.trialInit == 1
    ...
    agent.t     = agent.t  + agent.dt*(agent.st==0); % Only update during the interval
...
t2 = agent.t2; % round(agent.t2 * AGENT.cmprs);
...
if agent.t2 ~= 0 && agent.s ~= agent.ss
    pause
end
