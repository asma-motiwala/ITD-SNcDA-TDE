...
function [agent, history] = UPDATE_AGENT_A3(AGENT, agent, cue, action, history)

%%% Alternative representation 3:
%   Update elapsed time since interval onset only during interval.
%   After interval offset, only estimate of elapsed time since 
%       interval offset is updated.
...
if cue.trialInit == 1
    ...
    agent.t     = agent.t  + agent.dt*(agent.st==0);
